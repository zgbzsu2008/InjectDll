...
// DllMain函数
typedef BOOL(WINAPI *DllEntryProc)(HMODULE hModule, DWORD ul_reason_for_call, LPVOID lpReserved);
...
static void OutputLastError(const char *msg)
{
    LPSTR tmp;
    char *tmpmsg;
    FormatMessageA(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS, NULL, GetLastError(), MAKELANGID(LANG_NEUTRAL, SUBLANG_NEUTRAL), (LPSTR)&tmp, 0, NULL);
...
static void CopySections(const unsigned char *data, PIMAGE_NT_HEADERS old_headers, PMEMORYMODULE module)
{
    ...
    unsigned char *dest;
    PIMAGE_SECTION_HEADER section = IMAGE_FIRST_SECTION(module->headers); // 取首个节区
    for (i = 0; i < module->headers->FileHeader.NumberOfSections; i++, section++) {
        if (section->SizeOfRawData == 0) { // 文件中节区大小
            // section doesn't contain data in the dll itself, but may define uninitialized data
            size = old_headers->OptionalHeader.SectionAlignment;// 内存中分配节区最小值
            if (size > 0) {
...
static int BuildImportTable(PMEMORYMODULE module)
{
    ...
    if (directory->Size > 0) {
        // 导入表PIMAGE_IMPORT_DESCRIPTOR[] = {"模块字符串", ...}
        ...
        for (; !IsBadReadPtr(importDesc, sizeof(IMAGE_IMPORT_DESCRIPTOR)) && importDesc->Name; importDesc++) {
            ...
            if (importDesc->OriginalFirstThunk) {
                thunkRef = (POINTER_TYPE *)(codeBase + importDesc->OriginalFirstThunk); // ITN地址
                funcRef = (FARPROC *)(codeBase + importDesc->FirstThunk);
            } else {
                // no hint table
                thunkRef = (POINTER_TYPE *)(codeBase + importDesc->FirstThunk); // IAT地址
                funcRef = (FARPROC *)(codeBase + importDesc->FirstThunk);
            }
...
HMEMORYMODULE MemoryLoadLibrary(const void *data)
{
    ...
    // 2. 在"PEHeader.OptionalHeader.ImageBase"处分配"PEHeader.OptionalHeader.SizeOfImage"字节的空间
    ...
    // 3. 拷贝DOS头 + DOS STUB + PE头到headers地址处
    ...
    // 3. 拷贝节区(section)数据 文件->内存
    CopySections(data, old_header, result);

    // 4. 进程地址跟PE[ImageBase]不相等则 需要重定位
    locationDelta = (SIZE_T)(code - old_header->OptionalHeader.ImageBase);
    if (locationDelta != 0) {
        PerformBaseRelocation(result, locationDelta);
    }

    // 5. 加载依赖dll，并构建"PEHeader.OptionalHeader.DataDirectory.Image_directory_entry_import"导入表
