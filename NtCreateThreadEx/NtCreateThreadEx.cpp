...
int main()
{
    ...
    if (hProcess) {
        ...
        HANDLE hThread = NULL;
        auto NtCreateThreadEx = (pfnNtCreateThreadEx)GetProcAddress(GetModuleHandle(L"ntdll.dll"),"NtCreateThreadEx");
        NtCreateThreadEx(&hThread, 0x1FFFFF, NULL, hProcess,LoadLibraryAddress, p, FALSE, NULL, NULL, NULL, NULL);
        if (hThread) {
